...
public class Disruptor<T> {
    ...
    public RingBuffer<T> start() {
        ...
        //遍历执行consumerInfo，这个类里面封装着event
        for (final ConsumerInfo consumerInfo : consumerRepository) {
            //这个executor 就是我们 new disruptor 对象时传入的线程池
            // 所有的event都会在 handleEventsWith方法执行时 被封装成eventProcessor对象的
            //EventProcessor 对象实现了runnable，也就是说这是一个线程对象
            // event会走到 BatchEventProcessor 类中，
            // worker会走到 WorkProcessor 类中
    ...
    /**
     * 给 ringBuffer 中 添加processorSequences序列数组，并移除barrierSequences序列数组
     *
