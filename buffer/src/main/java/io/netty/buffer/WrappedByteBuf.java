 ...
 *
 * 另外的Bytebuf的包装类。
 * 这是一个非常重要的类。他的{@link #readerIndex()}和{@link #writerIndex()}在运行中的下标上不会进行调整，
 * 因为其内部的优化由{@link ByteBufUtil#writeAscii(ByteBuf, CharSequence)}
 * 和{@link ByteBufUtil#writeUtf8(ByteBuf, CharSequence)} 来完成的。
 *
 */
